...
function renderIcoonPreview(optie: IcoonOptie, size = 16, kleur = "currentColor") {
  if (optie.type === "emoji") {
    return <span style={{ fontSize: size }}>{optie.emoji}</span>
  }
  if (optie.component) {
    ...
    <div style={{ position: "relative" }}>
      ...
      <button
        ...
        style={{
          ...
          padding: "4px 10px", border: "1px solid #e5e7eb", borderRadius: "6px",
          backgroundColor: "white", cursor: "pointer", fontSize: "12px", color: "#374151",
          minWidth: "120px"
        }}
      >
        {geselecteerd
          ...
          <div style={{ maxHeight: "240px", overflowY: "auto", padding: "4px 8px 8px" }}>
            {(() => {
              ...
              return categorieen.map(cat => (
                <div key={cat}>
                  <p style={{ fontSize: "10px", fontWeight: "600", color: "#9ca3af", textTransform: "uppercase", margin: "8px 0 4px", letterSpacing: "0.05em" }}>
                    {cat}
                  </p>
            ...
            {resultaten.length === 0 && (
              <p style={{ fontSize: "12px", color: "#9ca3af", textAlign: "center", padding: "16px" }}>Geen iconen gevonden</p>
            )}
          </div>
        </div>
      )}
